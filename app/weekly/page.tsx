...
const emptyPlan = (): WeeklyPlan => {
  ...
  return { id: `week-${crypto.randomUUID()}`, week_start_date: new Date().toISOString().slice(0, 10), weekly_theme: "", top_3_priorities: "", owner: "", status: "planned", key_metric_target: "", actual_result: "", notes: "", created_at: now, updated_at: now, created_by: "" };
};

const emptyTask = (status: Task["status"] = "backlog"): Task => {
  const now = new Date().toISOString();
  return { id: `task-${crypto.randomUUID()}`, title: "", description: "", owner: "", priority: "medium", status, due_date: "", linked_strategy_pillar: "", linked_experiment: "", created_at: now, updated_at: now, created_by: "" };
};
  ...
  return (
    <div className="max-w-7xl p-6 lg:p-8">
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        ...
        <button onClick={() => setEditingTask(emptyTask("this_week"))} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 font-medium text-white hover:bg-blue-500"><Plus size={18} /> Add Task</button>
      </div>
      ...
      {tasksStore.records.length === 0 ? (
        <div className="rounded-lg border border-dashed border-slate-800 bg-slate-900 p-10 text-center"><p className="text-lg font-semibold text-slate-100">No tasks yet. Add your first execution task.</p><button onClick={() => setEditingTask(emptyTask("this_week"))} className="mt-5 rounded-lg bg-blue-600 px-4 py-2 font-medium text-white hover:bg-blue-500">Add Task</button></div>
      ) : (
