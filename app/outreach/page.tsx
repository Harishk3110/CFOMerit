          ...
          <aside className="rounded-lg border border-slate-800 bg-slate-900 p-5">
            {selected ? (
              <>
                ...
                <div className="mb-5 grid grid-cols-2 gap-3 text-sm"><Detail label="Score" value={`${selected.priority_score}/100`} /><Detail label="Owner" value={selected.owner || "-"} /><Detail label="Follow-up" value={selected.next_follow_up_at || "-"} /><Detail label="Source" value={selected.source || "-"} /></div>
                ...
                <div className="mb-5 grid grid-cols-2 gap-3"><button onClick={generateMessages} disabled={drafting} className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-60"><Sparkles size={16} /> {drafting ? "Generating" : "Generate"}</button><button onClick={markSent} className="rounded-lg border border-slate-700 px-3 py-2 text-sm text-slate-200 hover:bg-slate-800">Mark Manually Sent</button></div>
...
function LeadModal({ lead, exists, onCancel, onSave }: { lead: OutreachLead; exists: boolean; onCancel: () => void; onSave: (event: FormEvent<HTMLFormElement>) => void }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><div className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-lg border border-slate-800 bg-slate-950 p-6 shadow-2xl"><h2 className="mb-5 text-xl font-bold text-white">{exists ? "Edit Lead" : "Add Lead"}</h2><form onSubmit={onSave} className="space-y-4"><div className="grid grid-cols-2 gap-3"><Input name="first_name" label="First name" defaultValue={lead.first_name} required /><Input name="last_name" label="Last name" defaultValue={lead.last_name} required /></div><div className="grid grid-cols-2 gap-3"><Input name="company_name" label="Company" defaultValue={lead.company_name} /><Input name="role_title" label="Role" defaultValue={lead.role_title} /></div><div className="grid grid-cols-3 gap-3"><SelectField name="lead_type" label="Type" defaultValue={lead.lead_type} options={leadTypes} /><SelectField name="status" label="Status" defaultValue={lead.status} options={statuses} /><Input name="owner" label="Owner" defaultValue={lead.owner} /></div><div className="grid grid-cols-3 gap-3"><Input name="industry" label="Industry" defaultValue={lead.industry} /><Input name="location" label="Location" defaultValue={lead.location} /><Input name="next_follow_up_at" label="Follow-up" type="date" defaultValue={lead.next_follow_up_at?.slice(0, 10) || ""} /></div><Input name="linkedin_url" label="LinkedIn URL" defaultValue={lead.linkedin_url || ""} /><Input name="source" label="Source" defaultValue={lead.source} /><Textarea name="profile_summary" label="Profile summary" defaultValue={lead.profile_summary || ""} /><Textarea name="personalization_notes" label="Notes" defaultValue={lead.personalization_notes || ""} /><div className="grid grid-cols-2 gap-3"><Input name="pain_angle" label="Pain angle" defaultValue={lead.pain_angle || ""} /><Input name="merit_angle" label="Merit angle" defaultValue={lead.merit_angle || ""} /></div><div className="flex gap-3 pt-2"><button type="button" onClick={onCancel} className="flex-1 rounded-lg border border-slate-700 px-4 py-2 font-medium text-slate-300 hover:bg-slate-900">Cancel</button><button type="submit" className="flex-1 rounded-lg bg-blue-600 px-4 py-2 font-medium text-white hover:bg-blue-500">Save Lead</button></div></form></div></div>;
}
